...
package org.apache.spark.mllib.matrixdecomposition
 ...
 */
private[matrixdecomposition] case class InLinkBlock(
...
class ALS private (
   ...
   */
  def this() = this(-1, -1, 10, 10, 0.01, 1.0)
  ...
  def run(ratings: RDD[Rating]): MatrixFactorizationModel = {
    //get spark context of ratings RDD
    val sc = ratings.context

    //if numBlocks are -1, calculate numbers of blocks
    ...
    //create customer partitionner for user block and product block
    val userPartitioner = new ALSPartitioner(numUserBlocks)
    val productPartitioner = new ALSPartitioner(numProductBlocks)

    //create key-value pair RDD (partitionId, rating object)
 ...
 * Partitioner for ALS.
 */
private[matrixdecomposition] class ALSPartitioner(override val numPartitions: Int) extends Partitioner {
